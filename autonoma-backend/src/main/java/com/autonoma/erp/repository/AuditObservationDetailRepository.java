...
public interface AuditObservationDetailRepository extends JpaRepository<AuditObservationDetail, Long> {
    @Query("SELECT ad FROM AuditObservationDetail ad " +
           ...
           "AND (:ncrStatus IS NULL OR (:ncrStatus = 'All' AND (ad.ncrStatus IS NULL OR ad.ncrStatus != 'CLOSED')) OR ad.ncrStatus = :ncrStatus OR (:ncrStatus = 'OPEN' AND (ad.ncrStatus IS NULL OR ad.ncrStatus = 'OPEN' OR ad.ncrStatus = '')) ) " +
