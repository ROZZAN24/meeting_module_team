...
public interface MasterChecklistRepository extends JpaRepository<MasterChecklist, Long>, JpaSpecificationExecutor<MasterChecklist> {
    ...
    @Query("SELECT m FROM MasterChecklist m JOIN m.departments d WHERE (m.status IS NULL OR m.status != 'In Active') AND m.category = :category AND m.checkingPoint = :checkingPoint AND d.departmentName IN :departments AND (:id IS NULL OR m.id != :id)")
