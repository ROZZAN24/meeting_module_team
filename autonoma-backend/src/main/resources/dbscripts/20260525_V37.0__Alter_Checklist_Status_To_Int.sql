...
-- STEP 1: Add new INT columns alongside old ones only if they don't exist and old ones are still varchar/nvarchar
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'STATUS_INT')
   AND EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'STATUS' AND system_type_id IN (TYPE_ID('varchar'), TYPE_ID('nvarchar')))
BEGIN
    ALTER TABLE [dbo].[qms_checklist_master] ADD [STATUS_INT] INT NULL;
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'TASK_STATUS_INT')
   AND EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'TASK_STATUS' AND system_type_id IN (TYPE_ID('varchar'), TYPE_ID('nvarchar')))
BEGIN
    ALTER TABLE [dbo].[qms_checklist_master] ADD [TASK_STATUS_INT] INT NULL;
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'VERIFY_STATUS_INT')
   AND EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'VERIFY_STATUS' AND system_type_id IN (TYPE_ID('varchar'), TYPE_ID('nvarchar')))
BEGIN
    ALTER TABLE [dbo].[qms_checklist_master] ADD [VERIFY_STATUS_INT] INT NULL;
END
GO

-- STEP 2: Migrate data - convert string -> int codes using Dynamic SQL to prevent compile-time parser errors
...
-- STEP 5: Fix casing of created_at and updated_at -> CREATED_AT, UPDATED_AT safely
IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'created_at')
BEGIN
    EXEC sp_rename 'dbo.qms_checklist_master.created_at', 'CREATED_AT', 'COLUMN';
END
GO

IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.qms_checklist_master') AND name = 'updated_at')
BEGIN
