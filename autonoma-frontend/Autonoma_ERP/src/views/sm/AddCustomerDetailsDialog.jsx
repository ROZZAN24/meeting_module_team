...
export default function AddCustomerDetailsDialog({ open, handleClose, initialData }) {
  ...
  const fetchDetails = async (customerId) => {
    // In a real app, you would fetch the details for this customer
    // const response = await axios.get(`/api/sm/customer-details?customerId=${customerId}`);
  ...
  const handleSaveDetail = () => {
    ...
    const newRecord = {
      ...
      shipment: `SHP-${Math.floor(Math.random() * 1000)}`,
      ...formData
    };
