...
import {
  Typography, Button, Stack, Tooltip, IconButton, MenuItem, Checkbox, Grid, Box, Tabs, Tab, Card, CardContent, FormControlLabel, InputAdornment, Divider, Paper, Table, TableHead, TableRow, TableCell, TableBody, TableContainer, Chip, useTheme
} from '@mui/material';
import axios from 'utils/axios';
import {
  IconArrowLeft, IconDeviceFloppy, IconEraser, IconUser, IconBriefcase, IconSchool, IconCurrencyDollar, IconAddressBook, IconLock, IconStar, IconTrendingUp, IconPlus, IconTrash, IconCalendar, IconFileText
  ...
  const fetchApplicantDetails = useCallback(async () => {
    if (!applicantId) {
      ...
      try {
        ...
        setFormData(prev => ({
          ...prev,
          enRolledNo: data || 'ATS-2026-001',
          applicantDate: getTodayDateString()
        }));
      } catch (e) {
        console.error('Failed to fetch next enrolled number', e);
        setFormData(prev => ({ ...prev, enRolledNo: 'ATS-2026-001' }));
      } finally {
        setLoading(false);
      }
      return;
    ...
    <MainCard
      title={
        <Stack direction="row" alignItems="center" spacing={1.5}>
          <IconUser size={24} />
          <Typography variant="h3">{applicantId ? 'Edit Applicant Profile' : 'New Applicant Registration'}</Typography>
        </Stack>
      }
      secondary={
        <Stack direction="row" spacing={1.5}>
          ...
          <Tooltip title={shortcutTooltip('Save Candidate', 'Ctrl + S')}>
            <span>
              <Button variant="contained" startIcon={<IconDeviceFloppy size={18} />} onClick={handleSave} disabled={loading} sx={{ ...btnSave, px: 2.2, py: 0.75 }}>
                {loading ? 'Saving...' : 'Save'}
              </Button>
            </span>
          </Tooltip>
