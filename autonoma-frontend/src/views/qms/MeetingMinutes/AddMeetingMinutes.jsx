...
export default function AddMeetingMinutes() {
  ...
  const handleSave = async () => {
    ...
    const invalidDates = form.details.filter(
      (d) => d.processType === 'ACTION' && d.reviewDate && d.targetDate && d.reviewDate < d.targetDate
    );
    ...
    const pastDates = form.details.filter(
      (d) => d.processType === 'ACTION' && ((d.targetDate && d.targetDate < TODAY) || (d.reviewDate && d.reviewDate < TODAY))
    );
    ...
    const missingAssignments = form.details.filter((d) => d.processType === 'ACTION' && (!d.assignedBy || !d.assignedTo));
      ...
      try {
        ...
        if (editId) {
          if (isHost && !perms.write) {
            ...
            dispatch(
              openSnackbar({ open: true, message: 'Attendance out times updated Successfully...', variant: 'alert', severity: 'success' })
            );
          } else {
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  {[
                      ...
                      component: (
                        <BOSTextField
                          ...
                          sx={{
                            ...
                            '& .MuiInputBase-input': { fontWeight: 800, color: 'secondary.dark', py: 0.8 }
                          }}
                        />
                      )
                    },
                    ...
                    },
                    {
                      ...
                      component: (
                        <Autocomplete
                          fullWidth
                          options={meetingSchedules.filter(
                            (s) => s.status === 'OPEN' || s.status === 'AUTO CLOSED' || (editId && s.id === form.schedule?.id)
                          )}
                    ...
                    },
                    {
                      ...
                      component: (
                        <Autocomplete
                          ...
                          disabled={!perms.write}
                          renderInput={(params) => (
                            <BOSTextField {...params} placeholder="Select Chaired By" sx={{ '& .MuiInputBase-root': { py: 0 } }} />
                          )}
                        />
                      )
                    },
                    ...
                    <TableRow key={idx}>
                      <TableCell sx={{ width: '150px', whiteSpace: 'nowrap' }}>
                        <Typography variant="subtitle2" sx={{ fontWeight: 800, color: 'text.secondary', fontSize: '0.72rem' }}>
                          {row.label}
                        </Typography>
                      </TableCell>
          ...
          <Box sx={{ flex: 7.5, minWidth: 0, display: 'flex' }}>
            ...
            >
                    ...
                    {form.attendanceList.length === 0 ? (
                      <TableRow>
                        <TableCell
                          colSpan={7}
                          align="center"
                          sx={{ py: 12, fontStyle: 'italic', color: 'text.secondary', fontSize: '0.75rem' }}
                        >
                      ...
                      form.attendanceList.map((att, idx) => (
                        <TableRow key={idx} sx={{ '&:hover': { bgcolor: 'primary.lighter' }, transition: 'background 0.2s' }}>
                          ...
                          <TableCell sx={{ fontSize: '0.55rem', fontWeight: 800, color: 'text.secondary' }}>
                            {(att.employee?.department?.departmentName || 'N/A').toUpperCase()}
                          </TableCell>
                          <TableCell
                            sx={{
                              fontWeight: 800,
              ...
              <Box sx={{ mt: 1, textAlign: 'center', p: 0.5, bgcolor: 'error.lighter', borderRadius: 1.5 }}>
                <Typography variant="caption" color="error.dark" fontWeight={800} sx={{ fontSize: '0.6rem' }}>
                  *** Note: Please enter the host's outTime first before proceeding ***
                </Typography>
              </Box>
            </BOSFormSection>
          </Box>
        ...
        <Box>
          <BOSFormSection title="DISCUSSION & ACTION POINTS" icon={<IconMessageDots size={20} />} contentSx={{ p: 0 }}>
            <TableContainer
              component={Paper}
              elevation={0}
              sx={{ border: 'none', borderRadius: 2, overflowX: 'hidden', overflowY: 'auto', maxHeight: 600 }}
            >
              <Table
                stickyHeader
                size="small"
