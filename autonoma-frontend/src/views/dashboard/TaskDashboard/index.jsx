...
import {
  ...
  Stack, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, LinearProgress, IconButton, Button,
...
const StaticEmoji = ({ url, size = 44, style = {} }) => {
  return <img src={url} width={size} height={size} alt="emoji" style={{ objectFit: 'contain', filter: 'drop-shadow(0 4px 6px rgba(0,0,0,0.15))', ...style }} />;
};
...
const WorkloadView = ({ realWorkload, isDark }) => {
  ...
  const DataTable = ({ rows }) => (
    <TableContainer><Table>
      <TableHead sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.02)' : '#FFFFFF' }}>
        <TableRow>
          <TableCell sx={{ py: 1.5, borderBottom: `1px solid ${borderColor}` }}><Stack direction="row" alignItems="center" gap={1}><PersonOutlineRoundedIcon fontSize="small" sx={{ color: '#94A3B8' }} /><Typography variant="subtitle2" fontWeight={800} color="text.primary">Employee</Typography></Stack></TableCell>
          <TableCell sx={{ py: 1.5, borderBottom: `1px solid ${borderColor}`, width: '35%' }}><Stack direction="row" alignItems="center" gap={1}><Box sx={{ width: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><AssignmentRoundedIcon fontSize="small" sx={{ color: '#94A3B8' }} /></Box><Typography variant="subtitle2" fontWeight={800} color="text.primary">Workload</Typography></Stack></TableCell>
          <TableCell sx={{ py: 1.5, borderBottom: `1px solid ${borderColor}` }}><Stack direction="row" alignItems="center" gap={1}><FolderOpenRoundedIcon fontSize="small" sx={{ color: '#94A3B8' }} /><Typography variant="subtitle2" fontWeight={800} color="text.primary">Active Task</Typography></Stack></TableCell>
          <TableCell sx={{ py: 1.5, borderBottom: `1px solid ${borderColor}` }}><Stack direction="row" alignItems="center" gap={1}><CalendarTodayRoundedIcon fontSize="small" sx={{ color: '#94A3B8' }} /><Typography variant="subtitle2" fontWeight={800} color="text.primary">Total Days</Typography></Stack></TableCell>
          <TableCell sx={{ py: 1.5, borderBottom: `1px solid ${borderColor}` }}><Stack direction="row" alignItems="center" gap={1}><BookmarkBorderRoundedIcon fontSize="small" sx={{ color: '#94A3B8' }} /><Typography variant="subtitle2" fontWeight={800} color="text.primary">Status</Typography></Stack></TableCell>
          <TableCell align="right" sx={{ py: 1.5, borderBottom: `1px solid ${borderColor}` }}><Box sx={{ display: 'flex', justifyContent: 'flex-end' }}><MoreVertRoundedIcon fontSize="small" sx={{ color: '#94A3B8' }} /></Box></TableCell>
        </TableRow>
      </TableHead>
          ...
          <TableRow key={idx} hover sx={{ '& td': { borderBottom: idx === rows.length - 1 ? 'none' : `1px solid ${borderColor}` } }}>
            <TableCell sx={{ py: 1.5 }}>
              <Stack direction="row" alignItems="center" gap={2}>
                <Avatar sx={{ width: 32, height: 32, bgcolor: row.color, fontSize: '14px', fontWeight: 700, color: '#fff' }}>{row.user.charAt(0).toUpperCase()}</Avatar>
                <Box>
                  <Typography variant="subtitle2" fontWeight={800} color="text.primary">{row.user}</Typography>
                  <Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ fontSize: '0.65rem' }}>{row.user.toLowerCase().includes('admin') ? 'Administrator' : 'Developer'}</Typography>
                </Box>
              </Stack>
            </TableCell>
            <TableCell sx={{ py: 1.5 }}>
              <Box sx={{ pr: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
                <Box sx={{ width: 120 }}>
                  <Typography variant="subtitle2" fontWeight={900} color="text.primary" mb={0.2}>{row.percent}%</Typography>
                  <Typography variant="caption" color="text.secondary" fontWeight={600} display="block" sx={{ fontSize: '0.65rem' }}>{row.percent} / 100% Completed</Typography>
                </Box>
            ...
            <TableCell sx={{ py: 1.5 }}>
              <Chip icon={<AssignmentRoundedIcon sx={{ fontSize: '14px !important' }} />} label={row.tasks} size="small" sx={{ bgcolor: alpha(row.color, 0.1), color: row.color, fontWeight: 800, borderRadius: 2, px: 0.5, height: 24, '& .MuiChip-icon': { color: row.color } }} />
            </TableCell>
            <TableCell sx={{ py: 1.5 }}>
              <Chip icon={<CalendarTodayRoundedIcon sx={{ fontSize: '14px !important' }} />} label={`${row.days} Days`} size="small" sx={{ bgcolor: alpha(row.color, 0.1), color: row.color, fontWeight: 800, borderRadius: 2, px: 0.5, height: 24, '& .MuiChip-icon': { color: row.color } }} />
            </TableCell>
            <TableCell sx={{ py: 1.5 }}>
              <Chip icon={row.status === 'Critical' ? <ErrorRoundedIcon sx={{ fontSize: '16px !important' }} /> : <CheckCircleRoundedIcon sx={{ fontSize: '16px !important' }} />} label={row.status} size="small" sx={{ bgcolor: alpha(row.color, 0.1), color: row.color, fontWeight: 800, borderRadius: 6, px: 1, height: 24, '& .MuiChip-icon': { color: row.color } }} />
            </TableCell>
            <TableCell align="right" sx={{ py: 1.5 }}>
              <IconButton size="small" sx={{ color: '#94A3B8' }}><MoreVertRoundedIcon fontSize="small" /></IconButton>
            </TableCell>
          </TableRow>
        ))}
      </TableBody>
  ...
  return (
    <Box sx={{ p: 0 }}>
      <Grid container spacing={2.5} mb={2.5}>
        <Grid item xs={12} lg={8}>
          <Grid container spacing={2.5} sx={{ height: '100%' }}>
            {[
              { c: '#EF4444', bg: isDark ? '#2A161A' : '#FFF0F2', label: 'Critical', n: criticalCount, emoji: '1f6a8', data: [10, 25, 15, 40, 20, 50] },
              { c: '#3B82F6', bg: isDark ? '#17223B' : '#F0F5FF', label: 'Normal', n: normalCount, emoji: '1f44d', data: [20, 10, 30, 15, 40, 25] },
              { c: '#10B981', bg: isDark ? '#14251E' : '#F0FAF5', label: 'Healthy', n: healthyCount, emoji: '1f60e', data: [30, 40, 20, 50, 30, 60] }
            ].map((s, i) => (
              <Grid item xs={12} sm={4} key={i}>
                <Card sx={{ p: 2, pb: 1, display: 'flex', flexDirection: 'column', bgcolor: s.bg, border: `1px solid ${borderColor}`, boxShadow: 'none', height: '100%', borderRadius: 3 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, width: 40, height: 40 }}>
                      <Box sx={{ position: 'absolute', bottom: -2, width: 24, height: 6, bgcolor: s.c, filter: 'blur(8px)', opacity: 0.5, borderRadius: '50%' }} />
                      <NotoEmoji hex={s.emoji} size={40} />
                    </Box>
                    <Box>
                      <Typography variant="subtitle2" color={s.c} fontWeight={800} sx={{ lineHeight: 1 }}>{s.label}</Typography>
                      <Typography variant="h5" fontWeight={900} sx={{ lineHeight: 1.1, mt: 0.3 }} color="text.primary">{s.n}</Typography>
                      <Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ fontSize: '0.6rem', display: 'block', mt: 0.1 }}>Employees</Typography>
                    </Box>
                  </Box>
                  <Box sx={{ mt: 'auto', pt: 1 }}>
        ...
        <Grid item xs={12} lg={4}>
          <Card sx={{ height: '100%', background: isDark ? '#1E293B' : '#F8FAFC', border: `1px solid ${borderColor}`, boxShadow: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 2, borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
            <Box sx={{ zIndex: 2, flex: 1 }}>
              <Typography variant="h6" fontWeight={900} mb={0.5} color="text.primary">Team Performance <NotoEmoji hex="1f680" size={20} style={{ display: 'inline-block', verticalAlign: 'text-bottom' }} /></Typography>
              <Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ maxWidth: 200, display: 'block', lineHeight: 1.4, mb: 1.5 }}>
                Track workload, active tasks and productivity in real-time.
              </Typography>
              <Button variant="contained" endIcon={<TrendingUpRoundedIcon />} onClick={() => setViewAllOpen(true)} sx={{ bgcolor: '#6366F1', '&:hover': { bgcolor: '#4F46E5' }, borderRadius: 1.5, textTransform: 'none', fontWeight: 700, px: 2, py: 0.6, fontSize: '0.75rem' }}>
                View All
              </Button>
            </Box>
      ...
      <Dialog fullScreen open={viewAllOpen} onClose={() => setViewAllOpen(false)} TransitionComponent={Transition} PaperProps={{ sx: { bgcolor: isDark ? '#0F172A' : '#F8FAFC' } }}>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 3, borderBottom: `1px solid ${borderColor}`, bgcolor: isDark ? '#1E293B' : '#FFFFFF' }}>
          <Typography variant="h5" fontWeight={900}>All Employees Workload List</Typography>
          <IconButton onClick={() => setViewAllOpen(false)} sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9' }}><CloseRoundedIcon /></IconButton>
        </DialogTitle>
      ...
      <Grid container spacing={2.5}>
        <Grid item xs={12} md={5}>
          <Card sx={{ p: 3, bgcolor: isDark ? '#2A161A' : '#F5F3FF', border: 'none', display: 'flex', alignItems: 'center', gap: 3, height: '100%', borderRadius: 3, boxShadow: 'none', position: 'relative' }}>
            <Box sx={{ width: 64, height: 64, flexShrink: 0 }}><NotoEmoji hex="1f3c6" size={64} /></Box>
            <Box>
              <Typography variant="h6" fontWeight={900} color="text.primary" mb={0.5}>Great job team! 🎊</Typography>
              <Typography variant="body2" color="text.secondary" fontWeight={600} lineHeight={1.5}>{healthyEmployees} employees are performing great.<br />Let's keep the momentum going!</Typography>
        ...
        <Grid item xs={12} md={7}>
          <Grid container spacing={2.5} sx={{ height: '100%' }}>
            <Grid item xs={4}>
              <Card sx={{ p: 2.5, pb: 1.5, bgcolor: isDark ? '#1E293B' : '#FFFFFF', border: `1px solid ${borderColor}`, height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 3, boxShadow: 'none' }}>
                <Stack direction="row" alignItems="center" gap={1.5} mb={1}>
                  <Box sx={{ width: 40, height: 40, borderRadius: '30%', bgcolor: alpha('#8B5CF6', 0.15), color: '#8B5CF6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><TrendingUpRoundedIcon fontSize="small" /></Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ fontSize: '0.65rem' }}>Average Workload</Typography>
                    <Typography variant="subtitle1" fontWeight={900} color="#8B5CF6" sx={{ lineHeight: 1.2 }}>{avgWorkload}%</Typography>
                  </Box>
                </Stack>
                <Box sx={{ mt: 'auto', mx: -2 }}><ReactApexChart options={sparklineOptions('#8B5CF6')} series={[{ data: [20, 40, 30, 50, 40, 60] }]} type="line" height={30} width="100%" /></Box>
              </Card>
            </Grid>
            <Grid item xs={4}>
              <Card sx={{ p: 2.5, pb: 1.5, bgcolor: isDark ? '#1E293B' : '#FFFFFF', border: `1px solid ${borderColor}`, height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 3, boxShadow: 'none' }}>
                <Stack direction="row" alignItems="center" gap={1.5} mb={1}>
                  <Box sx={{ width: 40, height: 40, borderRadius: '30%', bgcolor: alpha('#3B82F6', 0.15), color: '#3B82F6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><AccessTimeRoundedIcon fontSize="small" /></Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ fontSize: '0.65rem' }}>Average Active Tasks</Typography>
                    <Typography variant="subtitle1" fontWeight={900} color="#3B82F6" sx={{ lineHeight: 1.2 }}>{avgActiveTasks}</Typography>
                  </Box>
                </Stack>
                <Box sx={{ mt: 'auto', mx: -2 }}><ReactApexChart options={sparklineOptions('#3B82F6')} series={[{ data: [10, 25, 20, 40, 30, 50] }]} type="line" height={30} width="100%" /></Box>
              </Card>
            </Grid>
            <Grid item xs={4}>
              <Card sx={{ p: 2.5, pb: 1.5, bgcolor: isDark ? '#1E293B' : '#FFFFFF', border: `1px solid ${borderColor}`, height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 3, boxShadow: 'none' }}>
                <Stack direction="row" alignItems="center" gap={1.5} mb={1}>
                  <Box sx={{ width: 40, height: 40, borderRadius: '30%', bgcolor: alpha('#10B981', 0.15), color: '#10B981', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><TrackChangesRoundedIcon fontSize="small" /></Box>
                  <Box>
                    <Typography variant="caption" color="text.secondary" fontWeight={700} sx={{ fontSize: '0.65rem' }}>Overall Productivity</Typography>
                    <Typography variant="subtitle1" fontWeight={900} color="#10B981" sx={{ lineHeight: 1.2 }}>{overallProductivity}</Typography>
                  </Box>
                </Stack>
                <Box sx={{ mt: 'auto', mx: -2 }}><ReactApexChart options={sparklineOptions('#10B981')} series={[{ data: [30, 50, 40, 60, 50, 70] }]} type="line" height={30} width="100%" /></Box>
              </Card>
            </Grid>
          </Grid>
        </Grid>
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        let tasksList = [];
        cl.forEach(a => { const name = getName(a.assignedToObj || a.employee || a.assignedTo); tasksList.push({ _status: a.status?.name || a.status?.statusName || 'Pending', _dueDate: a.checklistDate || a.assignedDate, _title: a.checklist?.checkingPoint || `Checklist #${a.id}`, _id: a.checklistNo || `CL-${a.id}`, _user: name, _rawDate: a.createdAt || a.createdDate || a.assignedDate || a.checklistDate, _hrs: a.estimatedHours || a.plannedHours || 8, _pageName: a.pageName || a.moduleName || 'Checklist' }); });
        mom.forEach(a => { const name = getName(a.assignedTo); tasksList.push({ _status: a.status || 'Open', _dueDate: a.targetDate, _title: a.discussedPoint || `MOM #${a.id}`, _id: a.momNo || a.actionId || `MOM-${a.id}`, _user: name, _rawDate: a.createdAt || a.createdDate || a.targetDate, _hrs: a.estimatedHours || 8, _pageName: a.pageName || a.moduleName || 'MOM Actions' }); });
        tk.forEach(t => { const name = getName(t.assignedTo); tasksList.push({ _status: t.ticketStatus || 'Open', _dueDate: t.dueDate || t.targetDate, _title: t.title || `Ticket ${t.ticketId || t.rowId}`, _id: t.ticketId || `TK-${t.rowId}`, _user: name, _rawDate: t.createdAt || t.createdDate || t.targetDate, _hrs: t.estimatedHours || 8, _pageName: t.pageName || t.moduleName || t.pageCode || 'Ticket' }); });
