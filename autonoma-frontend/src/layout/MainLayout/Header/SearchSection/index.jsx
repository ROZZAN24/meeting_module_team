...
const getAllPages = (items) => {
  ...
  const traverse = (node) => {
    if (node.type === 'item' && node.url) {
      pages.push({ title: node.title, url: node.url, pageCode: node.pageCode || '', id: node.id });
    }
...
export default function SearchSection() {
  ...
  useEffect(() => {
    // Sync navQuery with Redux value when not in '#' mode
    if (value !== undefined && !navQuery.startsWith('#')) {
      setNavQuery(value || '');
    }
                ...
                {({ TransitionProps }) => (
                  <Transitions type="zoom" {...TransitionProps} sx={{ transformOrigin: 'center left' }}>
                    <Card sx={{ bgcolor: 'background.default', border: 0, boxShadow: 'none' }}>
                      <Box sx={{ p: 2 }}>
                        <MobileSearch value={value} setValue={setValue} popupState={popupState} placeholder="Search in current page / Use # for page search..." />
                      </Box>
                    </Card>
                  </Transitions>
                )}
          ...
          renderOption={(props, option) => {
            ...
            return (
              <Box
                ...
                {...optionProps}
                sx={{
                  px: 1.5, py: 0.8, borderRadius: '6px', my: 0.2, mx: 0.5,
                ...
                {option.pageCode && (
                  <Typography
                    ...
                    sx={{
                      ...
                      color: 'primary.main',
                      px: 0.75, py: 0.25, borderRadius: '4px', fontWeight: 700, fontSize: '0.6875rem'
                    }}
                  >
                    {option.pageCode}
                  </Typography>
                                ...
                                <Stack spacing={0.6} key={field.id}>
                                  ...
                                  {field.type === 'autocomplete' ? (
                                    ...
                                    <Stack spacing={1.2}>
                                      ...
                                      <Stack spacing={0.5}>
                                        <Typography variant="caption" sx={{ color: '#9AA0B0', fontWeight: 600 }}>Consider Date?</Typography>
                                        <Select
                                          fullWidth size="small"
                                          variant="outlined"
