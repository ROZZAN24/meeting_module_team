    ...
    <MainCard
      ...
      secondary={
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Button variant="contained" color="secondary" size="small" onClick={() => handleVerify('Not Accepted')} disabled={!selectedRowId}>
            Not Accepted
          </Button>
          <Button
            variant="contained"
      ...
      {activeCount > 0 && (
        <Box sx={{ display: 'flex', gap: 0.5, mb: 2, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {filters.statuses?.map((s) => (
            <Chip key={s} label={s} size="small" color="warning" onDelete={() => toggleStatus(s)} />
          ))}
          {filters.assignTo && (
            <Chip label={`Assign To: ${filters.assignTo}`} size="small" color="info" onDelete={() => setFilter('assignTo', '')} />
          )}
          ...
          {filters.searchByValue && (
            <Chip label={`Value: ${filters.searchByValue}`} size="small" color="info" onDelete={() => setFilter('searchByValue', '')} />
          )}
      ...
      <Drawer anchor="right" open={drawerOpen} onClose={() => setDrawerOpen(false)} PaperProps={{ sx: { width: 320 } }}>
        ...
        <Box sx={{ overflowY: 'auto', flex: 1 }}>
          ...
          <FilterSection title="Consider Date?" open={openSections.considerDate} onToggle={() => toggleSection('considerDate')}>
            <FormControl>
              <RadioGroup value={filters.considerDate} onChange={(e) => setFilter('considerDate', e.target.value)}>
                {['All', 'Yes', 'No'].map((v) => (
                  <FormControlLabel
                    key={v}
                    value={v}
          ...
          <FilterSection title="Assign To" open={openSections.assignTo} onToggle={() => toggleSection('assignTo')}>
            <TextField
              ...
              onChange={(e) => setFilter('assignTo', e.target.value)}
            />
          </FilterSection>
          <Divider />
